      ...
      <div v-if="answer || error" class="answer-section">
        ...
        <div v-if="isInterrupted" class="interrupt-content">
          <div class="interrupt-header">
            ...
            <span class="interrupt-label">需要clarify：</span>
          </div>
...
export default {
  ...
  setup() {
    ...
    const clarifiedQuery = ref('')      // clarify后的查询文本
    ...
    const handleQuery = async () => {
      ...
      try {
        ...
        if (response.data.status === 'interrupt') {
          ...
          answer.value = response.data.message || '查询需要clarify:'
          error.value = ''
          loading.value = false
          console.log('🔄 查询被中断，等待clarify:', interruptInfo.value)
          return
        }
    ...
    /**
     * 处理resume查询
     * 当查询被interrupt后，使用clarify后的查询继续执行
     */
    const handleResume = async () => {
      if (!clarifiedQuery.value.trim()) {
        error.value = '请输入clarify后的查询内容'
        return
      }
