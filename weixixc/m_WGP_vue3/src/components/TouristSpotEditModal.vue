...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isAddMode = computed(() => {
      return !props.spot.id // 如果没有id，则是新增模式
    })
    ...
    const initializeFormData = () => {
      ...
      formData.value = {
        // tourist_spot 表字段 - 支持多种字段名映射
        name: props.spot.name || '',
        地址: props.spot.地址 || props.spot.address || '',
        图片链接: props.spot.图片链接 || props.spot.image_url || '',
        城市: props.spot.城市 || props.spot.city || '',
        介绍: props.spot.介绍 || props.spot.description || '',
        开放时间: props.spot.开放时间 || props.spot.opening_hours || '',
        评分: props.spot.评分 || props.spot.rating || '',
        门票: props.spot.门票 || props.spot.ticket_price || '',
        建议游玩时间: props.spot.建议游玩时间 || props.spot.suggested_duration || '',
        建议季节: props.spot.建议季节 || props.spot.suggested_season || '',
        小贴士: props.spot.小贴士 || props.spot.tips || '',
    ...
    const validateForm = () => {
      ...
      if (isAddMode.value) {
        ...
        if (!formData.value.lng_wgs84) {
          error.value = '经度不能为空'
          return false
        }
        if (!formData.value.lat_wgs84) {
          error.value = '纬度不能为空'
          return false
        }
      }
      ...
      try {
        ...
        const requestData = {
          tourist_spot: {
            ...
            介绍: formData.value.介绍?.trim() || '',
            开放时间: formData.value.开放时间?.trim() || '',
            图片链接: formData.value.图片链接?.trim() || '',
            评分: formData.value.评分?.trim() || '',
            建议游玩时间: formData.value.建议游玩时间?.trim() || '',
            建议季节: formData.value.建议季节?.trim() || '',
            门票: formData.value.门票?.trim() || '',
            小贴士: formData.value.小贴士?.trim() || '',
            城市: formData.value.城市?.trim() || ''
          },
          a_sight: {
            name: formData.value.name.trim(), // 使用相同的景点名称
            level: formData.value.level || '',
            address: formData.value.地址.trim(),
            所属城市: formData.value.城市?.trim() || '',
        ...
        // 详细调试信息
        console.log('=== 添加景点请求调试信息 ===')
        console.log('请求URL:', API_CONFIG.buildURL(API_CONFIG.endpoints.touristSpots.add))
        ...
        console.log('字段验证状态:', {
          ...
          lng_wgs84: requestData.lng_wgs84,
          lat_wgs84: requestData.lat_wgs84
        })

        console.log('发送' + (isAddMode.value ? '新增' : '更新') + '请求:', requestData)

        let response;

        if (isAddMode.value) {
          // 新增模式：使用POST请求
          const addUrl = API_CONFIG.buildURL(API_CONFIG.endpoints.touristSpots.add)
          console.log('发送POST请求到:', addUrl)
          ...
          response = await axios.post(addUrl, requestData, requestConfig)
        } else {
          // 编辑模式：使用PUT请求
          let updateUrl;
          ...
          response = await axios.put(updateUrl, requestData)
        }
        ...
        if (response.status === 200 || response.status === 201) {
          console.log('景点' + (isAddMode.value ? '新增' : '更新') + '成功:', response.data)
          const savedSpot = { ...props.spot, ...formData.value, ...response.data }
          emit('save', savedSpot)
          close()
        } else {
          ...
          if (err.response.status === 400) {
            if (err.response.data && typeof err.response.data === 'object') {
              ...
              if (err.response.data.errors) {
                ...
                errorMessage = `字段验证失败: ${errorFields.join(', ')}`
              } else if (err.response.data.message) {
                errorMessage = `数据验证失败: ${err.response.data.message}`
              } else if (err.response.data.detail) {
                errorMessage = `请求参数错误: ${err.response.data.detail}`
              } else {
                errorMessage = '请求数据格式不正确，请检查必填字段'
              }
            } else {
              errorMessage = '请求数据格式不正确，请检查必填字段'
            }
            ...
            errorMessage = err.response.data?.message || err.response.data?.detail || `服务器错误 (${err.response.status})`
          }
        } else if (err.request) {
