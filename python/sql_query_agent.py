...
class SQLQueryAgent:
    ...
    def __init__(self, system_prompt: Optional[str] = None):
        ...
        # 确保输出解析为字符串
        self.chain = self.agent | StructuredOutputParser.from_response_schemas([
            ResponseSchema(name="answer", description="你对问题的回答"),
            ResponseSchema(name="geojson", description="最终产生的sql查询得到的geojson（如果存在,否则为空）")
        ])
    ...
    def run_with_thought_chain(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            formatted_thought_chain = format_log_to_str(
                self.thought_chain_log) if self.thought_chain_log else ""
