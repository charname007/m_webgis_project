...
"""
    ...
    def analyze_intent(self, query_text: str) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"开始分析查询意图: {query_text}")
            ...
            if isinstance(intent_result, QueryIntentResult):
                ...
                self.logger.info(f"意图分析完成: {result_dict['query_type']} (置信度: {result_dict['confidence']})")
                return result_dict
            else:
                # 如果返回的不是Pydantic对象，尝试转换为字典
                result_dict = dict(intent_result) if hasattr(intent_result, '__dict__') else intent_result
